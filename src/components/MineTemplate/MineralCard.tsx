...
const MineralCard = ({ mineral }: { mineral: Mineral }) => {
    ...
    return (
        <div className="col-span-12 md:col-span-6 lg:col-span-4 xl:col-span-3 border border-lime-500 p-4 flex flex-col gap-1.5">
            ...
            <button
                disabled={progress > 0}
                className={`w-full py-2 bg-black text-white rounded ${progress > 0 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={() => handleAddMiningExperience(mineral.experience, mineral.delay)}>Mine
            </button>
        </div>
    )
}
